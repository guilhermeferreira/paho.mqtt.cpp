...
TEST_F(will_options_test, test_dflt_constructor) {
	...
	EXPECT_EQ(nullptr, c_struct.topicName);
...
TEST_F(will_options_test, test_move_constructor) {
	...
	EXPECT_EQ(DFLT_QOS, orgOpts.get_qos());
	EXPECT_EQ(DFLT_RETAINED, orgOpts.is_retained());

	EXPECT_EQ(nullptr, get_c_struct(orgOpts).topicName);
	EXPECT_EQ(0, get_c_struct(orgOpts).payload.len);
	EXPECT_EQ(nullptr, get_c_struct(orgOpts).payload.data);
}
