...
TEST_F(message_test, test_buf_constructor) {
	...
	EXPECT_NE(DFLT_DUP, c_struct.dup != 0);
}
...
TEST_F(message_test, test_validate_qos) {
	// message shouldn't accept QoS <min
	ASSERT_THROW(mqtt::message::validate_qos(-1), std::invalid_argument);

	for (int i=0; i<=2; ++i) {
		// mesage should accept valid QoS
		ASSERT_NO_THROW(mqtt::message::validate_qos(0));
	}

	// message shouldn't accept QoS >max
	ASSERT_THROW(mqtt::message::validate_qos(3) ,std::invalid_argument);
}
