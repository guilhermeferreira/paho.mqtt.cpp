...

{
	...
	int rc = MQTTAsync_subscribeMany(cli_, static_cast<int>(topicFilters.size()),
									 static_cast<char**>(&filts[0]), qos_array.get(), &opts.get_c_struct());
...
itoken_ptr async_client::subscribe(const std::string& topicFilter, QoS qos)
{
	...
	int rc = MQTTAsync_subscribe(cli_, const_cast<char*>(topicFilter.c_str()), static_cast<int>(qos), &opts.get_c_struct());
